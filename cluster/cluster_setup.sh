...
# `pypsa-eur` conda environment from envs/environment.yaml, and wires up the
# Gurobi token-server licence so the conda gurobipy can check out a token from
# any compute node.
###############################################################################
set -euo pipefail
...
try:
    ...
    env = gp.Env()           # checks out a token from nic5-login1
    env.dispose()
