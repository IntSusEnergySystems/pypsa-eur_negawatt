...
def Cumulative_emissions_sector(country):
 ...
 fig.update_layout(
    ...
    yaxis=dict(
        title=dict(
            text="Cumulative Emissions (MtCO2 eq)",
            font=dict(size=15)
        ),
        tickfont=dict(size=15)
    ),
...
def spline_plot(country):
 ...
 suff = pd.read_excel(f"results/suff/htmls/ChartData_{country}.xlsx", sheet_name="Chart 31", index_col=0,skiprows=2)
 ref = pd.read_excel(f"results/ref/htmls/ChartData_{country}.xlsx", sheet_name="Chart 31", index_col=0,skiprows=2)
 suff = suff.drop(columns=["Ambient heat"], errors="ignore")
 ref = ref.drop(columns=["Ambient heat"], errors="ignore")
...
def scenario_capacities(country):
    ...
    # Update layout
    fig.update_layout(height=800, width=1200, showlegend=True, title=f"Capacities for {country} in 2050", yaxis_title=unit,legend=dict(font=dict(size=15)))
    logo['y']=1.021
...
def storage_capacities(country):
    ...
    # Update layout
    fig.update_layout(height=500, width=1200, showlegend=True, title=f"Capacities for {country} in 2050", yaxis_title=unit,legend=dict(font=dict(size=15)))
    logo['y']=1.05
    num_cols = len(groups)
    for col in range(1, num_cols + 1):
...
#   ac_transmission_suff = capacities_suff_2050.loc['transmission lines', '2050']
#   # dc_transmission_suff = capacities_suff_2050.loc['DC Transmission lines', '2050']
#   transmission_suff = ac_transmission_suff
...
#   dc_transmission_ref = capacities_ref_2050.loc['transmission lines', '2050']
#   transmission_ref = dc_transmission_ref
