...
def process_network(cluster, opt, sector_opt, planning_horizon, country):
        ...
        if study == 'suff':
            ...
            if country != 'EU':
               ...
               filtered_non = H2_nonenergyy[[idx for idx in H2_nonenergyy.index if any(country in idx for country in countries)]]
               H2_nonenergyy = filtered_non.sum().sum()
               H2_industry = clever_industry.loc["Total Final hydrogen consumption in industry"]
               filtered_ind =H2_industry[[idx for idx in H2_industry.index if any(country in idx for country in countries)]]
               H2_industry = filtered_ind.sum().sum()
               naphtha_industry = clever_industry.loc["Non-energy consumption of oil for the feedstock production"]
               filtered_nap =naphtha_industry[[idx for idx in naphtha_industry.index if any(country in idx for country in countries)]]
               naphtha_industry = filtered_nap.sum().sum()
               oil_industry = clever_industry.loc["Total Final oil consumption in industry"]
               filtered_oil =oil_industry[[idx for idx in oil_industry.index if any(country in idx for country in countries)]]
               oil_industry = filtered_oil.sum().sum()
...
def prepare_emissions(cluster, opt, sector_opt,planning_horizon, country):
        ...
        if country == 'EU':
            value = -(n.snapshot_weightings.generators @
                  n.links_t.p1.filter(like="waste CHP")
                  ).sum()
        else:
            value = -(n.snapshot_weightings.generators @
                      n.links_t.p1.filter(like=country).filter(like="waste CHP")
                      ).sum()
        collection.append(
            pd.Series(
...
entry_label_mapping_c = {
    ...
    'oil refining emissions': {'label': 'oil refining emissions', 'source': 'MtCO2', 'target': 'emmoilref'},
