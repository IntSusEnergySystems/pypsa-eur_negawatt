...
def rename_techs_tyndp(tech):
    ...
    if "heat pump" in tech or "resistive heater" in tech:
        ...
        if study == "suff":
            ...
            return "CCUS"
    elif tech in ["biomass", "solid biomass", "solid biomass for industry", "biogas", "solid biomass transport", "biomass exports", "biogas exports","municipal solid waste","municipal solid waste transport","solid biomass powerplants"]:
    ...
    elif tech in ["SMR", "ammonia cracker", "Haber-Bosch", "BioSNG", "biomass to liquid","methanol","ammonia", "methanol exports", "ammonia exports","methanolisation","shipping methanol" ,"BioSNG CC","HVC to air","ammonia store","biomass to liquid CC","industry methanol","non-sequestered HVC","electrobiofuels"]:
...
def calculate_transmission_values(cluster, opt, sector_opt, planning_horizons):
    ...
    for planning_horizon in planning_horizons:
        ...
        for country in countries:
          filtered_ac = n.lines.bus0 == country
          filtered_ac_r = n.lines.bus1 == country
          combined_condition = filtered_ac | filtered_ac_r
          filtered_lines = n.lines[combined_condition]
          filtered_dc = (n.links.carrier == 'DC') & (n.links.bus0 == country) & (~n.links.index.str.contains('reversed'))
          filtered_dc_r = (n.links.carrier == 'DC') & (n.links.bus1 == country) & (~n.links.index.str.contains('reversed'))
...
def clustered_costs(countries):
 ...
 def rename_techs_ty(tech):
    tech = rename_techs(tech)
    if tech in ["synthetic fuels & techs","power-to-liquid","power-to-heat","power-to-gas","CCU","CCUS","DAC","boilers","load shedding"]:
        return "Uses"
...
def Investment_costs(countries,results):
    ...
    for country in countries:
      ...
      if country != 'EU':
       ...
       df=df
      df = df.rename(columns={'cluster': 'Costs','Unnamed: 3': 'tech', f'{cluster}': '2030',f'{cluster}.1': '2040',f'{cluster}.2': '2050'})
      df[['2030', '2040', '2050']] = df[['2030', '2040', '2050']].apply(pd.to_numeric, errors='coerce')
...
def rename_techs_tynd(tech):
    ...
    elif tech in ["biomass", "solid biomass", "solid biomass for industry", "biogas", "solid biomass transport", "biomass exports", "biogas exports","solid biomass import"]:
          return "Biomass"
...
def electricity_co2_price():
  ...
  for planning_horizon in planning_horizons:
    ...
    co2_value = abs(co2.loc["CO2Limit"])
    co2_values.append(co2_value)

    elec = n.buses_t.marginal_price.filter(regex="BE low voltage").sum().sum() / 8760
...
def plot_ch4_map(network):
    ...
    # gas_data = {}
    # planning_horizon = snakemake.params.planning_horizons
    # for country in countries:
...
def create_combined_chart_country(costs,investment_costs, capacities, s_capacities, country):
    ...
    if pypsa_plots["Sectoral Demands"] == True:
      ...
      with open(plot_demands_file_path, "r") as plot_demands_file:
        ...
        # combined_html += f"<div><h2>{display_country} - Sectoral Demands</h2>{plot_demands_html(full_html=False, include_plotlyjs='cdn')}</div>"
    # Create bar chart
    ...
    if pypsa_plots["Power Dispatch Winter"] == True:
      ...
      with open(plot_series_file_path, "r") as plot_series_file:
          ...
          # combined_html += f"<div><h2>{display_country} - Power Dispatch Winter</h2>{plot_series_html(full_html=False, include_plotlyjs='cdn')}</div>"
    with open(plot_series_file_path_simp, "r") as plot_series_file_simp:
        plot_series_simp_html = plot_series_file_simp.read()
        # combined_html += f"<div><h2>{display_country} - Power Dispatch Winter Simplified</h2>{plot_series_simp_html(full_html=False, include_plotlyjs='cdn')}</div>"
    if pypsa_plots["Power Dispatch Summer"] == True:
      ...
      with open(plot_series_file_path_sum, "r") as plot_series_file_sum:
          ...
          # combined_html += f"<div><h2>{display_country} - Power Dispatch Summer</h2>{plot_series_html_w(full_html=False, include_plotlyjs='cdn')}</div>"
    with open(plot_series_file_path_sum_simp, "r") as plot_series_file_sum_simp:
        ...
        # combined_html += f"<div><h2>{display_country} - Power Dispatch Summer Simplified</h2>{plot_series_html_w_simp(full_html=False, include_plotlyjs='cdn')}</div>"
    if pypsa_plots["Heat Dispatch Winter"] == True:    
      ...
      with open(plot_series_heat_file_path, "r") as plot_series_heat_file:
          ...
          # combined_html += f"<div><h2>{display_country} - Heat Dispatch Winter</h2>{plot_series_heat_html(full_html=False, include_plotlyjs='cdn')}</div>"
    if pypsa_plots["Heat Dispatch Summer"] == True:   
      ...
      with open(plot_series_heat_file_path_sum, "r") as plot_series_heat_file_sum:
          ...
          # combined_html += f"<div><h2>{display_country} - Heat Dispatch Summer</h2>{plot_series_heat_html_w(full_html=False, include_plotlyjs='cdn')}</div>"
    if pypsa_plots["Map Plots"] == True:
      ...
      with open(plot_map_path, "r") as plot_map_path:
          ...
          # combined_html += f"<div><h2>Map Plots</h2>{plot_map_html(full_html=False, include_plotlyjs='cdn')}</div>"
    if pypsa_plots["H2 Map Plots"] == True:
      ...
      with open(plot_map_h2_path, "r") as plot_map_h2_path:
          ...
          # combined_html += f"<div><h2>H2 Map Plots</h2>{plot_map_h2_html(full_html=False, include_plotlyjs='cdn')}</div>"
    if pypsa_plots["Gas Map Plots"] == True:
      ...
      with open(plot_map_ch4_path, "r") as plot_map_ch4_path:
          ...
          # combined_html += f"<div><h2>Gas Map Plots</h2>{plot_map_ch4_html(full_html=False, include_plotlyjs='cdn')}</div>"
    ...
    # if country == "BE":
    #   html_sections["costs.html"] += f"<div id='Electricity_and_CO2_Price'><h2>{display_country} - Electricity and CO2 Price</h2>{co2_plot.to_html(full_html=False, include_plotlyjs='cdn')}</div>"
