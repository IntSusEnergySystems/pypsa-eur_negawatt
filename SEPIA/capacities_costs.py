...
def prepare_costs_2020(country):
  n=pypsa.Network(f"resources/{study}/networks/base_s_adm_elec_.nc")   
  ...
  ac_lines_cap = capacities.loc[(capacities['cluster'] == 'lines') &
  (capacities.index == 'transmission lines'),
      '2020'
  ].sum()
  dc_lines_cap = capacities.loc[(capacities['cluster'] == 'links') &
  (capacities.index == 'transmission lines'),
      '2020'
  ].sum()
  ...
  transmission_lines = transmission_lines * 0.5
  techs = {
