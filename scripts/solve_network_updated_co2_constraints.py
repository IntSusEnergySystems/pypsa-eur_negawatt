...
def prepare_network(
    ...
    # Fix A: bound the co2 atmosphere store to prevent a free LP variable.
    # With e_nom=inf and e_min_pu=-1, the store energy variable has bounds
    # [-inf, +inf] which Gurobi's homogeneous barrier treats as a free variable,
    # causing the LP to report "Unbounded model". Setting e_nom=2e9 (well above
    # any realistic annual CO2 budget) and e_min_pu=0 (atmosphere cannot go
    # below zero) makes the bounds [0, 2e9] and removes the free variable.
    # Note: 0 * 2e9 = 0.0 (not nan, unlike 0 * inf).
    co2_atm_inf = n.stores[(n.stores.carrier == "co2") & np.isinf(n.stores.e_nom)].index
    if not co2_atm_inf.empty:
        n.stores.loc[co2_atm_inf, "e_nom"] = 2e9  # 2 Gt CO2, above any annual CO2 budget
        n.stores.loc[co2_atm_inf, "e_min_pu"] = 0.0  # atmosphere CO2 cannot go below zero

    # Fix C: non-extendable generators with p_nom=inf and p_min_pu=0 get a NaN
    # lower bound because p_min_pu * p_nom = 0 * inf = NaN in numpy. PyPSA/linopy
    # then skips the lower bound constraint, leaving p[t] unbounded below (-inf).
    # These are load-shedding generators (carrier="load") added in this function.
    # With noisy_costs making their marginal_cost positive (~3000 EUR/MWh), an
    # unbounded decrease in p[t] drives the LP objective to -inf (Unbounded model).
    # Fix: set p_nom to 1e6 MW (1 TW), safely above any realistic European demand,
    # so that p_min_pu * p_nom = 0 * 1e6 = 0.0 (not NaN) and the lower bound
    # constraint p[t] >= 0 is properly added to the LP.
    _load_inf_idx = n.generators[
        (~n.generators.p_nom_extendable)
        & np.isinf(n.generators.p_nom)
        & (n.generators.p_min_pu == 0.0)
    ].index
    if not _load_inf_idx.empty:
        logger.warning(
            f"Fix C: setting p_nom=1e6 for {len(_load_inf_idx)} non-extendable "
            f"generators with p_nom=inf and p_min_pu=0 to prevent NaN LP lower bounds."
        )
        n.generators.loc[_load_inf_idx, "p_nom"] = 1e6
...
def imposed_values_sequestration(n, config):
    """Impose values for carbon sequestration for ref scenario.

    Caps e_nom_max for all CO2 sequestered stores to prevent unbounded LP.
    Without a finite cap, extendable stores with e_nom_max=inf and negative
    marginal_cost create free variables that cause Gurobi to report Unbounded.
    Belgium uses its own specific sequestration potential; other countries are
    capped at the total EU potential from sector.co2_sequestration_potential.
    """
    if config["run"]["name"] == "ref":
        # Per-country caps for Belgium (from dedicated config key)
        be_caps = config.get("sequestration_potentia_BE", {})
        for year in [2030, 2040, 2050]:
            store_name = f"BE co2 sequestered-{year}"
            if store_name in n.stores.index and year in be_caps:
                n.stores.loc[store_name, "e_nom_max"] = be_caps[year] * 1e6

        # Cap all other extendable CO2 sequestered stores using total EU potential.
        # add_co2_sequestration_limit() is currently broken (wrong carrier_attribute)
        # so this direct e_nom_max cap is the effective constraint.
        seq_potential = config.get("sector", {}).get("co2_sequestration_potential", {})
        for year in [2030, 2035, 2040, 2045, 2050]:
            potential_t = seq_potential.get(year, 0) * 1e6  # Mt -> t CO2
            for country in ["DE", "FR", "GB", "NL"]:
                store_name = f"{country} co2 sequestered-{year}"
                if store_name in n.stores.index and n.stores.loc[store_name, "e_nom_extendable"]:
                    n.stores.loc[store_name, "e_nom_max"] = potential_t
    return n
